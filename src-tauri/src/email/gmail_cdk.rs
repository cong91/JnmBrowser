...
impl GmailCdkService {
  ...
  /// Helper to run an async future on the current tokio runtime.
...
impl EmailService for GmailCdkService {
  fn redeem_cdk(&self, cdk: &str) -> Result<EmailInfo, EmailServiceError> {
    ...
    let text = Self::block_on(async { response.text().await }).map_err(|e| {
      EmailServiceError::Network(format!("failed to read redeem response: {e}"))
    })?;
    ...
    let data: serde_json::Value =
      serde_json::from_str(&text).map_err(|e| {
        EmailServiceError::Internal(format!("failed to parse redeem response: {e}"))
      })?;
  ...
  fn poll_verification_code(
    ...
    loop {
      ...
      let poll_timeout =
        std::cmp::min(Duration::from_secs(POLL_INTERVAL_SECS), remaining);

      let client = Self::client();
      let result: Result<reqwest::Response, reqwest::Error> = Self::block_on(async {
        client
          .post(&url)
          .header("accept", "*/*")
          .header("content-type", "application/json")
      ...
      match result {
        Ok(resp) => {
          let text = Self::block_on(async { resp.text().await }).unwrap_or_default();
          let data: Result<VerificationCodeResponse, _> =
          ...
          match data {
            Ok(vcr) => match vcr.status.as_str() {
              ...
              "email_invalid" => {
                return Err(EmailServiceError::EmailInvalid(
                  vcr.message
                    .unwrap_or_else(|| "email invalid".into()),
                ));
              }
              "processing" => {
