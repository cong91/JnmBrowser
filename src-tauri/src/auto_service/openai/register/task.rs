...
/// A running registration task. The blocking task owns its cleanup guard; the
/// registry retains only the cancellation handle.
pub struct RegistrationTaskHandle {
  pub cancel_flag: Arc<AtomicBool>,
}

/// Global registry of running registration tasks.
static TASKS: Lazy<Mutex<HashMap<String, RegistrationTaskHandle>>> =
  Lazy::new(|| Mutex::new(HashMap::new()));

fn register_task(task_id: String, handle: RegistrationTaskHandle) {
  ...
  F: FnOnce() + Send + 'static,
{
  ...
  register_task(task_id.clone(), RegistrationTaskHandle { cancel_flag });
  start_tx.send(()).map_err(|_| {
    remove_task(&task_id);
    "Registration task ended before startup was released".to_string()
  })
}
...
mod tests {
  ...
  async fn spawn_registered_publishes_before_running_and_cleans_after_completion() {
    let task_id = format!("registration-task-{}", uuid::Uuid::new_v4());
